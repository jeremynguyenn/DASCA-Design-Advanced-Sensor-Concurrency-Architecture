...
// Sample data với workqueue/timer-based sampling (roadmap V.18), spinlock cho critical section (V.16), memory barrier (I.1)
...
// IRQ handler cho interrupt handling/deferred work (V.16)
...
static int probe(struct i2c_client *clnt, const struct i2c_device_id *id) {
    ...
    ret = regmap_read(regmap, MPU9250_WHO_AM_I, &who);
    if (ret < 0 || who != MPU9250_WHO_AM_I_VAL) {
        ret = -ENODEV;
        goto free_shared;
    }

    ret = regmap_write(regmap, 0x6A, 0x40); // Reset
...
// Buffer checks in read (V.22), eBPF cho profiling (V.21)