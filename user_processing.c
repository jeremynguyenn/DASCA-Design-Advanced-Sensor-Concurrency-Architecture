...
#include <syslog.h> // Bổ sung
#include <stdatomic.h> // Thêm cho atomic
#include "queue.h"
...
atomic_int queue_size = ATOMIC_VAR_INIT(0); // Bổ sung: Lock-free counter cho queue size
...
void publish_to_raw_queue(char *data, size_t len, int type) {
    ...
    atomic_fetch_add(&queue_size, 1); // Bổ sung: Lock-free increment
...
void *processing_thread(void *arg) {
    ...
    while (1) {
        // Bổ sung: Circuit breaker nếu queue_size > 100, skip (V.20)
        if (atomic_load(&queue_size) > 100) {
            syslog(LOG_WARNING, "Queue size > 100, skipping processing (circuit breaker)");
            sleep(1); // Throttle
            continue;
        }

        rcu_read_lock(); // Bổ sung: RCU cho lock-free reads (V.15) - Placeholder emulation
        ...
        atomic_fetch_sub(&queue_size, 1); // Bổ sung: Lock-free decrement
        free(msg);
        pthread_mutex_unlock(&raw_queue_mutex);
        pthread_rwlock_unlock(&queue_rwlock);
        rcu_read_unlock();
        ...
        if (have_mpu && have_bme && have_ens) {
            ...
            // Bổ sung: Simple Kalman filter cho temp fusion (từ web search example)
...
// Bổ sung: RCU cho lock-free reads (simple user-space emulation, từ liburcu-like)
#define rcu_read_lock() // Placeholder: synchronize_rcu() sau write nếu dùng full liburcu
#define rcu_read_unlock() 
// Trong processing_thread, thay rwlock_rdlock bằng rcu_read_lock nếu optimize (V.15)
// Bổ sung: Circuit breaker nếu queue_size > 100, skip (V.20): atomic_load(&queue_size) > 100 thì log và sleep
