...
#include <sys/types.h>
#include <sys/wait.h>
#include <semaphore.h> // Bổ sung cho sem_t
#include <syslog.h> // Bổ sung cho logging in daemon
#include "sensors.h"
...
static void *watchdog_thread_func(void *arg) {
    while (1) {
        ...
        // Bổ sung: Simple deadlock detection
...
int main() {
    ...
    openlog("sensor_app", LOG_PID, LOG_DAEMON); // Bổ sung: Logging cho daemon
