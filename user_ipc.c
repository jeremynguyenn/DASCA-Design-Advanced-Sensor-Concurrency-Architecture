...
#include <semaphore.h>
#include <syslog.h> // Bổ sung cho logging
#include "processing.h"
#include "queue.h"
#include <sys/socket.h> // Thêm cho IPC
#include <sys/un.h> // Thêm cho Unix socket

#define MPU_DEV "/dev/mpu9250"
#define BME_DEV "/dev/bme280"
#define ENS_DEV "/dev/ens160"
#define MAX_EVENTS 10
#define QUEUE_MAX 100 // Bổ sung: Threshold
    ...
    while (1) {
        ...
        for (int n = 0; n < nfds; n++) {
            if (events[n].data.fd == tfd) {
                ...
                // Bổ sung: Circuit breaker
                if (atomic_load(&queue_size) > QUEUE_MAX) {
                    syslog(LOG_WARNING, "Queue full, skipping read");
                    continue;
                }
...
// Bổ sung: Implementation cho ipc_thread sử dụng Unix Domain Socket
void *ipc_thread(void *arg) {
    int sock = socket(AF_UNIX, SOCK_STREAM, 0);
    struct sockaddr_un addr = {AF_UNIX, "/tmp/sensor_ipc.sock"};
    bind(sock, (struct sockaddr*)&addr, sizeof(addr));
    listen(sock, 5);
    while (1) {
        int client = accept(sock, NULL, NULL);
        // Read from processed_queue, send to client (GUI/Cloud)
        pthread_mutex_lock(&processed_queue_mutex);
        // ... send data ... (Giả sử send(processed_head->data, processed_head->len, 0);)
        if (processed_head) {
            send(client, processed_head->data, processed_head->len, 0);
            struct data_msg *tmp = processed_head;
            processed_head = processed_head->next;
            free(tmp);
        }
